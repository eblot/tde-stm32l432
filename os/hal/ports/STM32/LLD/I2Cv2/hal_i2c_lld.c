...
#define qEvt(posn,info) { ++i2cI; \
  i2cQ[i2cI & (QEVENTS-1)].code=(posn); \
  i2cQ[i2cI & (QEVENTS-1)].state=(i2cp->state); \
  i2cQ[i2cI & (QEVENTS-1)].mode=(i2cp->mode); \
  i2cQ[i2cI & (QEVENTS-1)].param=(info); }
#define qEvd(posn,val,info) { ++i2cI; \
  i2cQ[i2cI & (QEVENTS-1)].code=(posn); \
  i2cQ[i2cI & (QEVENTS-1)].state=(val); \
  i2cQ[i2cI & (QEVENTS-1)].mode=(i2cp->mode); \
  i2cQ[i2cI & (QEVENTS-1)].param=(uint16_t)(info); }
...
void i2cPrintQ(BaseSequentialStream *chp)
{
  ...
  {
    chprintf(chp, "I2C debug ring - write pointer currently %0d\r\n", i2cI & (QEVENTS-1));
  }
  for (i = 0; i < QEVENTS; i++)
  {
